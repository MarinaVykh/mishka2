...
Запуск локального сервера: `npm start` (если мы запускали сервер, то все автоматом записывается в папку build).

Запуск конкретной задачи: `npm start имя_задачи` (список задач смотри в `gulpfile.js`).

Отправка содержимого папки build в ветку gh-pages: 'npm run deploy'.
